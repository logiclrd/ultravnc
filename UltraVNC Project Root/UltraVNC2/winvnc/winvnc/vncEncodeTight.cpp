						   ...
						   const RECT &rect)
{
	...
	if (m_bufflen < rawDataSize) {
		...
		m_byte_buffer = new BYTE [rawDataSize+1];
		if (m_byte_buffer == NULL)
			...
			if (CheckSolidTile(source, dx, dy, dw, dh, &colorValue, FALSE)) {
				...
				for (int i = 0; i < 4; i++) {
					if (i == 2) {
						...
						Translate(source, m_byte_buffer, onePixel);
							  ...
							  int x, int y, int w, int h)
{
	...
	Translate(source, m_byte_buffer, r);
...
vncEncodeTight::SendMonoRect(BYTE *dest, int w, int h)
{
	...
	switch (m_remoteformat.bitsPerPixel) {
	case 32:
		EncodeMonoRect32((CARD8 *)m_byte_buffer, w, h);
	...
	case 16:
		EncodeMonoRect16((CARD8 *)m_byte_buffer, w, h);
	...
	default:
		EncodeMonoRect8((CARD8 *)m_byte_buffer, w, h);
...
vncEncodeTight::SendIndexedRect(BYTE *dest, int w, int h)
{
	...
	switch (m_remoteformat.bitsPerPixel) {
	case 32:
		EncodeIndexedRect32((CARD8 *)m_byte_buffer, w * h);
	...
	case 16:
		EncodeIndexedRect16((CARD8 *)m_byte_buffer, w * h);
...
vncEncodeTight::SendGradientRect(BYTE *dest, int w, int h)
{
	...
	if (m_usePixelFormat24) {
		FilterGradient24(m_byte_buffer, w, h);
		len = 3;
	} else if (m_remoteformat.bitsPerPixel == 32) {
		FilterGradient32((CARD32 *)m_byte_buffer, w, h);
		len = 4;
	} else {
		FilterGradient16((CARD16 *)m_byte_buffer, w, h);
		len = 2;
	}
							 ...
							 int zlibLevel, int zlibStrategy)
{
	if (dataLen < TIGHT_MIN_TO_COMPRESS) {
		memcpy(dest, m_byte_buffer, dataLen);
		return dataLen;
	}
	...
	pz->next_in = (Bytef *)m_byte_buffer;
	pz->avail_in = dataLen;
...
{
	CARD8 *data = (CARD8 *)m_byte_buffer;
	CARD8 c0, c1;
	int i, n0, n1;
...
{																			  \
	CARD##bpp *data = (CARD##bpp *)m_byte_buffer;								  \
	CARD##bpp c0, c1, ci;													  \
	int i, n0, n1, ni;														  \
																			  \
	c0 = data[0];															  \
...
vncEncodeTight::DetectSmoothImage24 (int w, int h)
{
	...
	while (y < h && x < w) {
		for (d = 0; d < h - y && d < w - x - DETECT_SUBROW_WIDTH; d++) {
			for (c = 0; c < 3; c++) {
				left[c] = (int)m_byte_buffer[((y+d)*w+x+d)*4+off+c] & 0xFF;
			}
			for (dx = 1; dx <= DETECT_SUBROW_WIDTH; dx++) {
				for (c = 0; c < 3; c++) {
					pix = (int)m_byte_buffer[((y+d)*w+x+d+dx)*4+off+c] & 0xFF;
...
{																			  \
	...
	while (y < h && x < w) {												  \
		for (d = 0; d < h - y && d < w - x - DETECT_SUBROW_WIDTH; d++) {	  \
			pix = ((CARD##bpp *)m_byte_buffer)[(y+d)*w+x+d]; 					  \
			if (endianMismatch) {											  \
				pix = Swap##bpp(pix);										  \
			}																  \
			for (c = 0; c < 3; c++) {										  \
			...
			for (dx = 1; dx <= DETECT_SUBROW_WIDTH; dx++) { 				  \
				pix = ((CARD##bpp *)m_byte_buffer)[(y+d)*w+x+d+dx];				  \
				if (endianMismatch) {										  \
					pix = Swap##bpp(pix);									  \
				}															  \
				sum = 0;													  \
...
{
	if (m_remoteformat.bitsPerPixel == 32) {
		CARD32 *src = (CARD32 *)&m_byte_buffer[y * w * sizeof(CARD32)];
		if (m_usePixelFormat24) {
		...
		// 16 bpp assumed.
		CARD16 *src = (CARD16 *)&m_byte_buffer[y * w * sizeof(CARD16)];
