...
BOOL
vncEncoder::SendCursorShape(VSocket *outConn, remoteDesktop *desktop)
{
	BOOL success=false;
	int returnvalue=desktop->SendCursorShape(m_use_xcursor ,m_use_richcursor);
	if (returnvalue==1)
	{
		FixCursorMask(desktopsharedmem->mbits, NULL, desktopsharedmem->width, desktopsharedmem->bmHeight, desktopsharedmem->bmWidthBytes);
		success = SendXCursorShape(outConn, desktopsharedmem->mbits,
								   desktopsharedmem->xHotspot, desktopsharedmem->yHotspot,
								   desktopsharedmem->width, desktopsharedmem->height);
	}
	else if (returnvalue==2)
	{
		FixCursorMask(desktopsharedmem->mbits, desktopsharedmem->cbits, desktopsharedmem->width, desktopsharedmem->height, desktopsharedmem->bmWidthBytes);
		success = SendRichCursorShape(outConn, desktopsharedmem->mbits, desktopsharedmem->cbits,
									  desktopsharedmem->xHotspot, desktopsharedmem->yHotspot,
									  desktopsharedmem->width, desktopsharedmem->height);
	}
	else {
