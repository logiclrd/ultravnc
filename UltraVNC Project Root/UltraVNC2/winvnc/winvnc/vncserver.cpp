...
vncServer::ServerUpdateTracker::add_changed(const rfb::Region2D &rgn) {
	...
	critsec l(&m_server->CriticalSection1);
...
vncServer::vncServer()
{
	...
	serverSharedmem->m_poll_fullscreen = FALSE;
	serverSharedmem->m_poll_foreground = FALSE;
	serverSharedmem->m_poll_undercursor = TRUE;

	serverSharedmem->m_poll_oneventonly = FALSE;
	serverSharedmem->m_MaxCpu=20;
	serverSharedmem->m_poll_consoleonly = TRUE;

	serverSharedmem->m_driver = FALSE;
	serverSharedmem->m_hook = FALSE;
	m_virtual = FALSE;
	sethook=false;
	...
	m_lock_on_exit = 0;
	m_connect_pri = 0;
	serverSharedmem->m_disableTrayIcon = FALSE;
	serverSharedmem->m_AllowEditClients = FALSE;

	// Set the input options
	m_enable_remote_inputs = TRUE;
	serverSharedmem->m_disable_local_inputs = FALSE;
	...
	strcpy(serverSharedmem->m_szWindowName, "");
...
vncServer::Authenticated(vncClientId clientid)
{
	...
	{
	// only lock during client iteration
		critsec l(&CriticalSection1);
	...
	#ifdef _DEBUG
			char			szText[256];
			sprintf(szText," ++++++ new remoteDesktop 1\n");
			SetLastError(0);
			OutputDebugString(szText);		
	#endif
	...
	if (m_desktop == NULL)
	{
		#ifdef _DEBUG
			char			szText[256];
			sprintf(szText," ++++++ new remoteDesktop 2\n");
			SetLastError(0);
			OutputDebugString(szText);		
		#endif
		m_desktop = new remoteDesktop();
		if (m_desktop == NULL)
			{
				client->Kill();
				authok = FALSE;
		...
		m_desktop->m_bufferpointer->MultiMonitors(1);
		if (Secondary()) m_desktop->m_bufferpointer->MultiMonitors(2);


        DWORD startup_status = 0;
		if ((startup_status = m_desktop->Init()) != 0)
				{
	...
	client->SetBuffer((m_desktop->m_bufferpointer));
...
//
void vncServer::ListAuthClients(_clientlist *cl)
{
	vncClientList::iterator i;
	critsec l(&CriticalSection1);
	cl->size=0;
	for (i = m_authClients.begin(); i != m_authClients.end(); i++)
	{
		...
		if (client->GetRepeaterID() && (strlen(client->GetRepeaterID()) > 0) ) {
			...
			szDescription[255] = '\0';
			strcpy_s(cl->szDescription[cl->size],256,szDescription);
			cl->size++;
		} else {
			strcpy_s(cl->szDescription[cl->size],256,client->GetClientName());
			cl->size++;
		}
	}
}
...
// adzm 2009-07-05
void vncServer::ListUnauthClients(_clientlist *cl)
{
	vncClientList::iterator i;
	critsec l(&CriticalSection1);
...
{
	omni_mutex_lock l(m_notifyLock);
...
vncServer::UpdateMouse(POINT *IN_point)
{
	...
	{
		// Post the update
		GetClient(*i)->UpdateMouse(IN_point);
	}
}
...
{
	serverSharedmem->m_disable_local_inputs = disable;
}
...
vncServer::VerifyHost(const char *hostname) {
	omni_mutex_lock l(m_protectblacklistLock);
...
{
    memcpy(serverSharedmem->m_szWindowName, szName, 32);
}
...
{
	sethook=true;
	serverSharedmem->m_driver = enable;
}
	
void vncServer::Hook(BOOL enable)
{
	sethook=true;
	serverSharedmem->m_hook=enable;
}
...
{
	serverSharedmem->m_fXRichCursor = fEnable;
}

BOOL
vncServer::SetDisableTrayIcon(BOOL disableTrayIcon)
{
	if (disableTrayIcon != serverSharedmem->m_disableTrayIcon)
	{
		serverSharedmem->m_disableTrayIcon = disableTrayIcon;
	}
	return TRUE;
}
...
{
	return serverSharedmem->m_disableTrayIcon;
}
BOOL
vncServer::SetAllowEditClients(BOOL AllowEditClients)
{
	if (AllowEditClients != serverSharedmem->m_AllowEditClients)
	{
		serverSharedmem->m_AllowEditClients = AllowEditClients;
	}
	return TRUE;
}
...
{
	return serverSharedmem->m_AllowEditClients;
}
