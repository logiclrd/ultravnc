...
vncEncodeZlib::~vncEncodeZlib()
{
	...
	if (m_byte_buffer2 != NULL)
	{
		delete [] m_byte_buffer2;
		m_byte_buffer2 = NULL;
	}
...
vncEncodeZlib::EncodeOneRect(BYTE *source,BYTE *source2, BYTE *dest, const RECT &rect,VSocket *outConn)
{
		...
		if (m_queueEnable && source2 && dataSize>Firstrun)
			{
				...
				memcpy(dest+sz_rfbFramebufferUpdateRectHeader,m_byte_buffer,rawDataSize);
	...
	UINT newsize;
	SoMoMu=PURE_ZLIB;
	if (m_byte_buffer2 && source2 && dataSize>Firstrun)
		{
			Translate(source2,m_byte_buffer2, rect);
			newsize=PrepareXOR(m_byte_buffer,m_byte_buffer2,rect);
		}
	switch (SoMoMu)
	{
		case SOLID_COLOR:
			{
				...
				memcpy(dest+sz_rfbFramebufferUpdateRectHeader,m_byte_buffer,compStream.avail_in);
		...
		case MONO_COLOR:
			{
				...
				if (m_queueEnable)
				{
				memcpy(dest+sz_rfbFramebufferUpdateRectHeader,m_byte_buffer,newsize);
		...
		compStream.next_in = m_byte_buffer;
...
inline UINT
vncEncodeZlib::PrepareXOR(BYTE * m_byte_buffer,BYTE * m_byte_buffer2,RECT rect)
{
	...
	unsigned char *sourceptr =m_byte_buffer;
	unsigned char *source2ptr = m_byte_buffer2;
	unsigned char *sourceposptr = m_byte_buffer2;
...
//USE XOR AGAINST PREVIOUS IMAGE
	{
		...
		bool temparray[8];
		int byte_nr=0;
		sourceptr = m_byte_buffer;
		source2ptr = m_byte_buffer2;
		for (int i=0; i<rawDataSize;i=i+BytePerPixel)
		{
		...
		if ((maskDataSize+(k)*BytePerPixel)<rawDataSize)
		{
			...
			memcpy(sourceptr,m_Maskbuffer,maskDataSize);
			sourceptr+=maskDataSize;
			memcpy(sourceptr,m_byte_buffer2,k*BytePerPixel);
