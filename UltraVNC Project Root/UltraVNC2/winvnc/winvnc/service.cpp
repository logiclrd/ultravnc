...
static int pad2()
{
	...
	if (GetModuleFileName(NULL, exe_file_name, MAX_PATH))
		{
		char* p = strrchr(exe_file_name, '\\');
		*p = '\0';
		}

    strcpy_s(app_path,MAX_PATH, exe_file_name);
	strcat_s(app_path,MAX_PATH, "\\uvnc_session.exe -service");
...
BOOL SetSeCreateGlobalPrivilegePrivileges(VOID) {
  DWORD dwPID;
  HANDLE hProcess;
  HANDLE hToken;
  LUID Luid;
  ...
  if ((hProcess = OpenProcess(PROCESS_ALL_ACCESS, FALSE, dwPID)) == NULL) return FALSE;
  if (OpenProcessToken(hProcess, TOKEN_ALL_ACCESS, &hToken) == 0) return FALSE;
  if ((LookupPrivilegeValue(NULL, "SeCreateGlobalPrivilege", &Luid)) == 0) return FALSE;
  tpDebug.PrivilegeCount = 1;
  tpDebug.Privileges[0].Luid = Luid;
  tpDebug.Privileges[0].Attributes = SE_PRIVILEGE_ENABLED;
  if ((AdjustTokenPrivileges(hToken, FALSE, &tpDebug, sizeof(tpDebug), NULL, NULL)) == 0) return FALSE;
...
{
	SetTBCPrivileges();
	//SetSeCreateGlobalPrivilegePrivileges();
	pad2();
	...
	HANDLE threadHandle;
	DWORD dw;
	threadHandle = CreateThread(NULL, 0,Start_service_non_desktop_part, NULL, 0, &dw);
