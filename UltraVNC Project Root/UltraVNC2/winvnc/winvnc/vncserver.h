...
// - Propagating updates from the local remoteDesktop object
//   to all the connected clients
// - Propagating mouse movements and keyboard events from
//   clients to the local remoteDesktop
...
public:
	...
	virtual void ListAuthClients(_clientlist *cl);
	virtual void WaitUntilAuthEmpty();

	virtual void KillUnauthClients();
	virtual void ListUnauthClients(_clientlist *cl); // adzm 2009-07-05
	...
	virtual char *GetWindowName() { return serverSharedmem->m_szWindowName; };
	virtual remoteDesktop* GetDesktopPointer() {return m_desktop;}
	...
	omni_mutex			m_notifyLock;
	omni_mutex			m_protectblacklistLock;
	UINT				m_port;
...
public:
	...
	virtual void PollUnderCursor(BOOL enable) {serverSharedmem->m_poll_undercursor = enable;};
	virtual BOOL PollUnderCursor() {return serverSharedmem->m_poll_undercursor;};
	virtual void PollForeground(BOOL enable) {serverSharedmem->m_poll_foreground = enable;};
	virtual BOOL PollForeground() {return serverSharedmem->m_poll_foreground;};
	virtual void PollFullScreen(BOOL enable) {serverSharedmem->m_poll_fullscreen = enable;};
	virtual BOOL PollFullScreen() {return serverSharedmem->m_poll_fullscreen;};

	virtual void Driver(BOOL enable);
	virtual BOOL Driver() {return serverSharedmem->m_driver;};
	virtual void Hook(BOOL enable);
	virtual BOOL Hook() {return serverSharedmem->m_hook;};
	virtual void Virtual(BOOL enable) {m_virtual = enable;};
	virtual BOOL Virtual() {return m_virtual;};
	virtual void SetHookings();

	virtual void PollConsoleOnly(BOOL enable) {serverSharedmem->m_poll_consoleonly = enable;};
	virtual BOOL PollConsoleOnly() {return serverSharedmem->m_poll_consoleonly;};
	virtual void PollOnEventOnly(BOOL enable) {serverSharedmem->m_poll_oneventonly = enable;};
	virtual BOOL PollOnEventOnly() {return serverSharedmem->m_poll_oneventonly;};
	virtual void MaxCpu(LONG maxcpu) {serverSharedmem->m_MaxCpu = maxcpu;};
	virtual LONG MaxCpu() {return serverSharedmem->m_MaxCpu;};
	...
	*/
	...
	virtual void EnableRemoveWallpaper(const BOOL enable) {serverSharedmem->m_remove_wallpaper = enable;};
	virtual BOOL RemoveWallpaperEnabled() {return serverSharedmem->m_remove_wallpaper;};
	// Removal of desktop composit desktop, etc
	virtual void EnableRemoveAero(const BOOL enable) {serverSharedmem->m_remove_Aero = enable;};
	virtual BOOL RemoveAeroEnabled() {return serverSharedmem->m_remove_Aero;};
	...
	virtual BOOL BlankMonitorEnabled() {return serverSharedmem->m_fBlankMonitorEnabled;};
	virtual void BlankMonitorEnabled(BOOL fEnable) {serverSharedmem->m_fBlankMonitorEnabled = fEnable;};
	virtual BOOL BlankInputsOnly() {return serverSharedmem->m_fBlankInputsOnly;}; //PGM
	virtual void BlankInputsOnly(BOOL fEnable) {serverSharedmem->m_fBlankInputsOnly = fEnable;}; //PGM
	...
	virtual BOOL IsXRichCursorEnabled() {return serverSharedmem->m_fXRichCursor;}; 
	...
	// sf@2002 - Turbo Mode
	virtual void TurboMode(BOOL fEnabled) { serverSharedmem->m_TurboMode = fEnabled; };
	virtual BOOL TurboMode() { return serverSharedmem->m_TurboMode; };
	...
	virtual BOOL CaptureAlphaBlending(){return serverSharedmem->m_fCaptureAlphaBlending;};
	virtual void CaptureAlphaBlending(BOOL fEnabled){serverSharedmem->m_fCaptureAlphaBlending = fEnabled;};
	virtual BOOL BlackAlphaBlending(){return serverSharedmem->m_fBlackAlphaBlending;};
	virtual void BlackAlphaBlending(BOOL fEnabled){serverSharedmem->m_fBlackAlphaBlending = fEnabled;};
...
//	omni_mutex			m_clientsLock;
	...
	// check ok, protect delete desktop
