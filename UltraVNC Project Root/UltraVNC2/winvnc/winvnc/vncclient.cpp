...
// It uses a vncBuffer and is passed the remoteDesktop and
...
extern comm_serv ClearKeyStateFn;
extern comm_serv keyEventFn;
extern comm_serv mouse_eventFn;
extern comm_serv SendInputFn;
...
vncClientUpdateThread::Init(vncClient *client)
{
	...
	m_client = client;
	critsec l(m_client->GetUpdateLock());
	m_active = TRUE;
...
vncClientUpdateThread::Trigger()
{
	...
	if (m_client->m_disable_protocol == 0) {
		vnclog.Print(LL_INTINFO, VNCLOG(" ******************************* SetEvent Trigger\n"));
		SetEvent(m_client->m_signal_event);
		//m_signal->signal();
	}
}
...
vncClientUpdateThread::Kill()
{
	...
	m_active=FALSE;
	vnclog.Print(LL_INTINFO, VNCLOG(" ******************************* SetEvent Kill\n"));
	SetEvent(m_client->m_signal_event);
	//m_signal->signal();
}
...
vncClientUpdateThread::EnableUpdates(BOOL enable)
{
	...
	m_enable = enable;
	vnclog.Print(LL_INTINFO, VNCLOG(" ******************************* SetEvent Enable Updates\n"));
	SetEvent(m_client->m_signal_event);
	//m_signal->signal();
...
//#endif
		...
		{
			// for some reason this need to be removed, before it existed ... strange
			//critsec l(m_client->GetUpdateLock());
...
//				m_sync_sig->broadcast();
				...
				unsigned long now_sec, now_nsec;
				get_time_now(&now_sec, &now_nsec);
				vnclog.Print(LL_INTINFO, VNCLOG(" ******************************* waitevent\n"));
				WaitForSingleObject(m_client->m_signal_event,1000);
				//m_signal->wait();
				vnclog.Print(LL_INTINFO, VNCLOG(" ******************************* waitevent done\n"));
			}
			}
			else
			{
...
//				m_sync_sig->broadcast();
				...
				vnclog.Print(LL_INTINFO, VNCLOG(" ******************************* waitevent\n"));
				//m_signal->wait();
				WaitForSingleObject(m_client->m_signal_event,10000);
				vnclog.Print(LL_INTINFO, VNCLOG(" ******************************* waitevent done\n"));
			...
			{
				vnclog.Print(LL_INTWARN, VNCLOG("m_client->m_incr_rgn.clear\n"));
				m_client->m_incr_rgn.clear();
			}
		...
		if ((m_client->m_encodemgr.m_scrinfo.framebufferHeight == m_client->m_encodemgr.m_buffer->sharedbuffermem->m_scrinfo.framebufferHeight) &&
			(m_client->m_encodemgr.m_scrinfo.framebufferWidth == m_client->m_encodemgr.m_buffer->sharedbuffermem->m_scrinfo.framebufferWidth) &&
			(m_client->m_encodemgr.m_scrinfo.format.bitsPerPixel == m_client->m_encodemgr.m_buffer->sharedbuffermem->m_scrinfo.format.bitsPerPixel))
		{
...
vncClientThread::FilterClients_Ask_Permission()
{
	...
	_acceptdialog ad;
	strcpy_s(ad.PeerName,32,m_socket->GetPeerName());
	ad.QueryAccept=m_server->QueryAccept();
	ad.QueryIfNoLogon=m_server->QueryIfNoLogon();
	...
	if (!InitVersion())
	{
		// adzm 2009-07-05
		{
			...
			if (m_client->GetRepeaterID() && (strlen(m_client->GetRepeaterID()) > 0) ) {
				_snprintf(szInfo, 255, "connection from/to using %s! protocol error", m_client->GetRepeaterID());
			} else {
				_snprintf(szInfo, 255, "connection from/to %s! protocol error", m_client->GetClientName());
			}

			szInfo[255] = '\0';

			NotifyBalloon.Call_Fnction(szInfo, NULL);
		}
		m_server->RemoveClient(m_client->GetClientId());
		...
		if (m_server->AutoReconnect())
		{
			...
			MENU_ADD_CLIENT_MSG_fn( 1111, 1111,m_server);
		}
...
//				vnclog.Print(LL_SOCKERR, VNCLOG("Update asked for region %i %i %i %i %i\n"),update.tl.x,update.tl.y,update.br.x,update.br.y,m_client->m_SWOffsetx);
...

				{
					...
					if (!msg.fur.incremental)
					{
						...
						m_client->m_encodemgr.m_buffer->m_desktop->QueueRect();
					}					
		...
		case rfbKeyEvent:
			...
			{				
				if (m_client->m_keyboardenabled)
				{
					...
					keyEventdata ked;
					ked.keysym=msg.ke.key;
					ked.down=(0 != msg.ke.down);
					ked.jap=m_client->m_jap;
					ked.version=vncService::VersionMajor();
					ked.from_service=vncService::RunningFromExternalService();
...
//							vnclog.Print(LL_INTINFO, VNCLOG("########mouse_event :%i %i \n"),x,y);
						...
						{//second or spanned
							...
							{							
								...
								int xx=msg.pe.x-serverSharedmem->XVIRTUALSCREEN+ (m_client->m_SWOffsetx+m_client->m_ScreenOffsetx);
								int yy=msg.pe.y-serverSharedmem->YVIRTUALSCREEN+ (m_client->m_SWOffsety+m_client->m_ScreenOffsety);
								evt.mi.dx = (xx * 65535) / (serverSharedmem->CXVIRTUALSCREEN-1);
								evt.mi.dy = (yy* 65535) / (serverSharedmem->CYVIRTUALSCREEN-1);
		...
		case rfbSetScale: // Specific PalmVNC SetScaleFactor
			...
			m_client->m_nScale = msg.ssc.scale;
			{
				...
				m_client->m_encodemgr.m_buffer->m_desktop->MouseRect();
...
// Modif cs@2005
	...
	m_szHost = NULL;
	m_hostPort = 0;
	m_signal_event=CreateEvent(NULL,false,true,NULL);
}
...
{
	//This is already locked in the remoteDesktopsynk
	//
	if (m_clipboard_text) {
		free(m_clipboard_text);
		m_clipboard_text = 0;
...
{
	if (lock) critsec l(GetUpdateLock());
	//if (lock) 
...
vncClient::SetNewSWSize(long w,long h,BOOL Desktop)
{
	...
	m_oldmousepos.br.x=0;
	m_oldmousepos.br.y=0;
	m_oldmousepos.tl.x=0;
	m_oldmousepos.tl.y=0;
