...
bool vncDesktop::FastDetectChanges(rfb::Region2D &rgn, rfb::Rect &rect, int nZone, bool fTurbo)
{
	...
	PixelEngine.PixelCaptureEngineInit(m_hrootdc, m_hmemdc, m_membitmap, m_fCaptureAlphaBlending && !m_Black_window_active, 
		                           m_DIBbits, desktopsharedmem->m_scrinfo.format.bitsPerPixel / 8, m_bytesPerRow);
...
vncDesktop::vncDesktop()
{
	...
	m_videodriver=NULL;
	desktopsharedmem->m_videodriver=NULL;
	m_ScreenOffsetx=0;
	m_ScreenOffsety=0;
	m_hookdriver=false;
    ...
    desktopsharedmem->m_bIsInputDisabledByClient = false;
	m_input_desktop = 0;
	m_home_desktop = 0;
	idle_counter=0;
...
{
	if (InputDesktopSelected2())
		return TRUE;
	vnclog.Print(LL_INTINFO, VNCLOG("InitDesktop...\n"));
	return SelectDesktop(NULL, &m_input_desktop);
}
								...
								if (DriverFound)
									{
										...
										desktopsharedmem->multi_monitor=false;
										if (requested_multi_monitor && nr_monitors>1) desktopsharedmem->multi_monitor=true;
									}
							}
					if (hUser32) FreeLibrary(hUser32);
				}//VIDEOBUFFER
...
//	}
	...
	if (m_hrootdc == NULL) {
		...
		{
			m_hrootdc = CreateDC(("DISPLAY"),desktopsharedmem->mymonitor[0].device,NULL,NULL);
			m_ScreenOffsetx=desktopsharedmem->mymonitor[0].offsetx;
			m_ScreenOffsety=desktopsharedmem->mymonitor[0].offsety;
		}
		else
		{
			m_hrootdc = GetDC(NULL);
			m_ScreenOffsetx=desktopsharedmem->mymonitor[2].offsetx;
			m_ScreenOffsety=desktopsharedmem->mymonitor[2].offsety;;
		}
		if (m_hrootdc == NULL) {
	...
	if (desktopsharedmem->multi_monitor && !VideoBuffer()) m_bmrect = rfb::Rect(0, 0,desktopsharedmem->mymonitor[2].Width,desktopsharedmem->mymonitor[2].Height);
	else if (!VideoBuffer()) m_bmrect = rfb::Rect(0, 0,desktopsharedmem->mymonitor[0].Width,desktopsharedmem->mymonitor[0].Height);
...
vncDesktop::SetPixFormat()
{
 // If we are using a memory bitmap then check how many planes it uses
  ...
  if (!m_DIBbits) {
	...
	desktopsharedmem->m_scrinfo.format.trueColour = m_bminfo.truecolour;
	desktopsharedmem->m_scrinfo.format.bigEndian = 0;

	// Set up the native buffer width, height and format
	desktopsharedmem->m_scrinfo.framebufferWidth = (CARD16) (m_bmrect.br.x - m_bmrect.tl.x);		// Swap endian before actually sending
	desktopsharedmem->m_scrinfo.framebufferHeight = (CARD16) (m_bmrect.br.y - m_bmrect.tl.y);	// Swap endian before actually sending
	desktopsharedmem->m_scrinfo.format.bitsPerPixel = (CARD8) m_bminfo.bmi.bmiHeader.biBitCount;
	desktopsharedmem->m_scrinfo.format.depth        = (CARD8) m_bminfo.bmi.bmiHeader.biBitCount;

	// Calculate the number of bytes per row
	m_bytesPerRow =desktopsharedmem-> m_scrinfo.framebufferWidth * desktopsharedmem->m_scrinfo.format.bitsPerPixel / 8;
...
vncDesktop::SetPixShifts()
{
	...
	MaskToMaxAndShift(redMask, desktopsharedmem->m_scrinfo.format.redMax, desktopsharedmem->m_scrinfo.format.redShift);
	MaskToMaxAndShift(greenMask, desktopsharedmem->m_scrinfo.format.greenMax, desktopsharedmem->m_scrinfo.format.greenShift);
	MaskToMaxAndShift(blueMask, desktopsharedmem->m_scrinfo.format.blueMax, desktopsharedmem->m_scrinfo.format.blueShift);
...
DWORD
vncDesktop::Init(remoteServer *server)
{
...
vncDesktop::CaptureMouse(BYTE *scrBuff, UINT scrBuffSize)
{
	...
	if (GetIconInfo(m_hcursor, &IconInfo))
	{
		...
		if (CursorPos.x<=m_bmrect.tl.x || CursorPos.y<=m_bmrect.tl.y || CursorPos.x>=m_bmrect.br.x || CursorPos.y>=m_bmrect.br.y) 
		{
			...
			m_cursorpos.br.x=16;
			m_cursorpos.br.y=16;
			desktopsharedmem->m_cursorpos_rect.left=m_cursorpos.tl.x;
			desktopsharedmem->m_cursorpos_rect.top=m_cursorpos.tl.y;
			desktopsharedmem->m_cursorpos_rect.right=m_cursorpos.br.x;
			desktopsharedmem->m_cursorpos_rect.bottom=m_cursorpos.br.y;
			}
...
vncDesktop::CopyToBuffer(const rfb::Rect &rect, BYTE *destbuff, UINT destbuffsize)
{
	...
	y_inv = desktopsharedmem->m_scrinfo.framebufferHeight-rect.tl.y-(rect.br.y-rect.tl.y);
	...
	if (m_DIBbits == NULL) {
		...
		int bytesPerPixel = desktopsharedmem->m_scrinfo.format.bitsPerPixel / 8;
...
BOOL vncDesktop::InitVideoDriver()
{
	...
	desktopsharedmem->m_videodriver=(void*)m_videodriver;
	...
	if (nr_monitors==1)
		{
			m_ScreenOffsetx=desktopsharedmem->mymonitor[0].offsetx;
			m_ScreenOffsety=desktopsharedmem->mymonitor[0].offsety;
			m_videodriver->VIDEODRIVER_start(desktopsharedmem->mymonitor[0].offsetx,desktopsharedmem->mymonitor[0].offsety,desktopsharedmem->mymonitor[0].Width,desktopsharedmem->mymonitor[0].Height);
		}
	if (nr_monitors>1)
		{
			m_ScreenOffsetx=desktopsharedmem->mymonitor[2].offsetx;
			m_ScreenOffsety=desktopsharedmem->mymonitor[2].offsety;
			m_videodriver->VIDEODRIVER_start(desktopsharedmem->mymonitor[2].offsetx,desktopsharedmem->mymonitor[2].offsety,desktopsharedmem->mymonitor[2].Width,desktopsharedmem->mymonitor[2].Height);
		}		
...
void vncDesktop::SetBlockInputState(bool newstate)
{
	...
	if (m_server->BlankMonitorEnabled())
    {
		...
		desktopsharedmem->m_bIsInputDisabledByClient = newstate;
		state=!block_input();
...
bool vncDesktop::block_input()
{
	int Blockinput_val;
	BOOL returnvalue;
	if(desktopsharedmem->m_bIsInputDisabledByClient || m_server->LocalInputsDisabled())
	{
		Blockinput_val=true;
	}
	else
	...
	if (old_Blockinput!=Blockinput_val)
	{
		CARD32 state;
...
int vncDesktop::SendCursorShape(bool m_use_xcursor, bool m_use_richcursor)
{
	...
	if (bmMask.bmWidthBytes * bmMask.bmHeight>(64*64*4))
    {
		DeleteObject(IconInfo.hbmMask);
		return FALSE;
    }
	...
	BOOL success = GetBitmapBits(IconInfo.hbmMask,
								 bmMask.bmWidthBytes * bmMask.bmHeight, desktopsharedmem->mbits);
	DeleteObject(IconInfo.hbmMask);
	...
	if (!isColorCursor && m_use_xcursor) {
		desktopsharedmem->bmHeight=bmMask.bmHeight;
		desktopsharedmem->height=height;
		desktopsharedmem->width=width;
		desktopsharedmem->bmWidthBytes=bmMask.bmWidthBytes;
		desktopsharedmem->xHotspot=IconInfo.xHotspot;
		desktopsharedmem->yHotspot=IconInfo.yHotspot;
		return 1;
	}
	else if (m_use_richcursor) {
		int cbits_size = width * height * 4;
		if (cbits_size>64*64*4) {		
			return FALSE;
		}
		if (!GetRichCursorData(desktopsharedmem->cbits, hcursor, width, height)) {
			vnclog.Print(LL_INTINFO, VNCLOG("remoteDesktop::GetRichCursorData() failed.\n"));
			return FALSE;
		}
		desktopsharedmem->bmHeight=bmMask.bmHeight;
		desktopsharedmem->height=height;
