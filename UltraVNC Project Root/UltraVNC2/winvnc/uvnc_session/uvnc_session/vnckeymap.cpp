...
DWORD WINAPI Cadthread(LPVOID lpParam)
{
	...
	GetVersionEx(&osvi);
	if (osvi.dwMajorVersion==6 && osvi.dwMinorVersion >= 1) return 0;
	HDESK desktop=NULL;
...
//Full path needed, sometimes it just default to system32
	...
	strcpy_s(mycommand,MAX_PATH,"");
	strcat_s(mycommand,MAX_PATH,WORKDIR);//set the directory
	strcat_s(mycommand,MAX_PATH,"\\");
	strcat_s(mycommand,MAX_PATH,"cad.exe");
...
DWORD WINAPI SimulateCtrlAltDelThread(LPVOID lpParam)
{
	HDESK old_desktop = GetThreadDesktop(GetCurrentThreadId());
	HDESK winlogon_desk = 0;
	// Switch into the Winlogon desktop
	if (!SelectDesktop("Winlogon", &winlogon_desk))
	{
		return FALSE;
	}
...
public:
  ...
  for (int j = 0; j < sizeof(latin1DeadChars); j++) {
    ...
    if (s != -1) {
      ...
      if (nchars < 0) {
        //vnclog.Print(LL_INTWARN, "Found dead key 0x%x '%c'",latin1DeadChars[j], latin1DeadChars[j]);
  ...
  void SimulateCtrlAltDel()
  {
	  ...
	  myhandle=CreateThread(NULL,0,SimulateCtrlAltDelThread,NULL,0,&dw);
	  if (myhandle) CloseHandle(myhandle);
  }


  void keyEvent(unsigned int keysym, bool down, bool jap,int version,bool from_service,comm_serv *sas_eventFn)
  {
	...
	if (keysymDead!=0 && down)
	{
	 ...
	 if (s == -1)
	 {
        ...
        // not a single keypress - try synthesizing dead chars.
			{
			  ...
			  unsigned char a0=ascii/100;
			  ascii=ascii%100;
			  unsigned char a1=ascii/10;
			  ascii=ascii%10;
			  unsigned char a2=ascii;
      ...
      //vnclog.Print(LL_INTINFO,"latin-1 key: keysym %d(0x%x) vkCode 0x%x down %d capslockOn %d\n",keysym, keysym, vkCode, down, capslockOn);
...
//      //vnclog.Print(LL_INTINFO,
  //                "keyboard key: keysym %d(0x%x) vkCode 0x%x ext %d down %d\n",
    //               keysym, keysym, vkCode, extendedMap[keysym], down);
      ...
      {
		//vnclog.Print(LL_INTINFO,"CAD\n");
		// If running under Vista and started from Session0 in Application mode
		if (version>=6 && from_service )
		{
			      //vnclog.Print(LL_INTINFO,"Vista and runnning as system -> CAD\n");
				sas_eventFn->Call_Fnction_no_feedback();
		...
		{
			//vnclog.Print(LL_INTINFO, "Vista and runnning as user -> Taskmgr\n");
			WinExec("taskmgr.exe", SW_SHOWNORMAL);
		}
		else if (version<6 && from_service )
		{
			//vnclog.Print(LL_INTINFO,"Not Vista and runnning as system, use old method\n");
			SimulateCtrlAltDel();
		}
		else if (version<6)
		{
			//vnclog.Print(LL_INTINFO,"Not Vista and runnning as user -> Taskmgr\n");
			WinExec("taskmgr.exe", SW_SHOWNORMAL);
		}
        return;
      }
...
private:
  ...
  std::map<unsigned int,bool> extendedMap;
} key_mapper;

void vncKeymap::keyEvent(unsigned long keysym, bool down,bool jap,int version,bool from_service,comm_serv *sas_event)
{
  key_mapper.keyEvent(keysym, down,jap,version,from_service,sas_event);
}
